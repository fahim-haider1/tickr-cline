...
async function canAccessTask(userId: string, taskId: string) {
  ...
  const isMember =
    ws.ownerId === userId ||
    ws.members.some((m) => m.userId === userId); // allow any member; tighten if needed
...
export async function GET(_req: NextRequest, { params }: { params: { taskId: string } }) {
  try {
    ...
    const access = await canAccessTask(userId, params.taskId);
...
export async function PATCH(req: NextRequest, { params }: { params: { taskId: string } }) {
  try {
    ...
    if (body.description !== undefined) data.description = body.description ?? null; // subtitle
    if (body.details !== undefined) data.details = body.details ?? null;             // long description
